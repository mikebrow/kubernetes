...
func TestPullImage(t *testing.T) {
	...
	imageRef, _, err := fakeManager.PullImage(kubecontainer.ImageSpec{Image: "busybox"}, nil, nil)
	assert.NoError(t, err)
...
func TestRemoveImage(t *testing.T) {
	...
	_, _, err = fakeManager.PullImage(kubecontainer.ImageSpec{Image: "busybox"}, nil, nil)
	assert.NoError(t, err)
...
func TestPullWithSecrets(t *testing.T) {
	...
	for description, test := range tests {
		...
		_, _, err = fakeManager.PullImage(kubecontainer.ImageSpec{Image: test.imageName}, test.passedSecrets, nil)
		require.NoError(t, err)
...
func TestPullThenListWithAnnotations(t *testing.T) {
	...
	_, _, err = fakeManager.PullImage(imageSpec, nil, nil)
	assert.NoError(t, err)
